...
rnwFile =  paste(simp$dirs$simp, "/", simp$sim$folder, "/cluster/common/", "craftyr_report_RegGlobMax_", 
		"RUN", "-SEED.Rnw", sep="")
...
simp$sim$id 	<- c(paste(preserve$run, "-", preserve$seed, sep=""))
texFile =  paste(simp$dirs$simp, "/", simp$sim$folder, "/cluster/", preserve$task,"/", "craftyr_report_RegGlobMax_", 
		"RUN", "-SEED.tex", sep="")
