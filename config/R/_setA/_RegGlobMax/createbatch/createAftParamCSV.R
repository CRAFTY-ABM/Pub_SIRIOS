...
for (aft in afts) {
	...
	for (scenario in simp$batchcreation$scenarios) {
		for (aftscenario in simp$batchcreation$aftscenarios) {
			for (multifunctionality in simp$batchcreation$multifunctionality) {
				for (giStage in giStages) {
					for (guStage in guStages) {	
						for (gup in simp$batchcreation$guprobs) {
							for (sd in simp$batchcreation$heterogeneitySD) {
						...
						d <- c(d,tData[tData$Scenario == scenario & tData$AftScenario == aftscenario & 
												tData$AFT == aft,c(-1,-2,-3)] * giFactor * guFactor)
				
						d["givingInDistributionSD"] <- sd
						d["givingUpDistributionSD"] <- sd
								
						d["productionCsvFile"] <- paste(simp$batchcreation$versiondirs$production, 
								"/production/", aft, "/AftProduction_", aft, "_", multifunctionality, "_", 
								capitalSensitivity, ".csv", sep="")
						
						d["givingUpProb"] <- gup #tData[tData$Scenario == scenario & tData$AFT == aft, c("givingUpProb")]
