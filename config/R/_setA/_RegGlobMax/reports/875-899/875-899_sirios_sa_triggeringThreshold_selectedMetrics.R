	...
	data$TriggeringThreshold <- data$TriggeringThreshold * 100 
	...
	visualise_lines(simp, data_selected, x_column = "TriggeringThreshold", y_column="Value", title = NULL,
			...
			facet_column = "Facet", facet_ncol = 1, filename = paste("SA_Triggering_SelectedMetrics", setsimp$sim$id, 
					setsimp$sim$id, sep="_"),
			alpha = simp$fig$alpha, ggplotaddons = list(
					ggplot2::xlab("Triggering Threshold in % of service demand"),
