...
for (set in unique(metrics[,1])) {
	...
	if (nointeractioneffects) {
		...
		simp$fig$height			<- 920
		simp$fig$width			<- 920
		filename 				<-  paste("crafty_netsens_analysis_doe_effects_600-615_selectedMetrics_nointeraction", set, sep="_")
		numcol					<- 2
	} else {
		indices <- rep(TRUE, length(rownames(fx)))
		effectdata <- setNames(fx$effects, rownames(fx))
		filename 				<-  paste("crafty_netsens_analysis_doe_effects_600-615_selectedMetrics", set, sep="_")
		numcol					<-  3
	}
