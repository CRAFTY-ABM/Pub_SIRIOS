...
metriccolnamesAll = c("VarChangesLu", "ConsPatches_NC", "ConsConnectivity","MaxUnderSupply", "MaxOverSupply",
		...
		"VarChangesCells", "DivLuShannon", "ConsPatches_C", "ConsProp_C", "ConsProp_NC",
		"UnderSupply_Meat", "UnderSupply_Cereal", "UnderSupply_Timber", "NumActions", "NumActionsNC",
...
metricColnamesFocus = c("VarChangesLu", "VarChangesCells",
		"MaxUnderSupply", "MaxOverSupply", "UnderSupply_Cereal", "UnderSupply_Timber","UnderSupply_Meat",
		"RegUnderSupply_Timber",
		"ConsPatches_NC", "ConsProp_NC", "ConsConnectivity", "NumActions", "NumActionsNC", "DivSupplyPerRegSimpson"
...
if (nointeractioneffects) {
	...
	simp$fig$height			<- 920
	simp$fig$width			<- 920
	filename 				<-  "crafty_netsens_analysis_doe_effects_3params_280-295_metricFocus_nointeraction_v2"
	numcol					<- 2
} else {
	indices <- rep(TRUE, length(rownames(fx)))
	effectdata <- setNames(fx$effects, rownames(fx))
	filename 				<-  "crafty_netsens_analysis_doe_effects_3params_280-295_metricFocus_v2"
	numcol					<-  3
}
...
shdoe::shdoe_visualise_interaction_effects(dexp = simp, 
		...
		substitutions = substitutions, 
		filename = "crafty_netsens_analysis_doe_effects_3params_280-295_metricUninteresting_v2",
		ggplotaddons = list(ggplot2::theme(legend.position="bottom"),
