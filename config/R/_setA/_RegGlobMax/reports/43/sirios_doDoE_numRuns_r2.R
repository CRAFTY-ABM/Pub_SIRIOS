...
dexp$doe$visual$gammaend <- 0.2

mdata[,"MaxOverSupply_Cereal-Meat-Timber"] <- NULL

# plot required number of runs
output_visualise_initFigure(simp, outdir = paste(simp$dirs$output$figures, "lines", sep="/"), 
		filename = "RequiredNumberOfRuns_R2_2040", ensurePath = TRUE)
shdoe::shdoe_visualise_requiredNumberOfRuns(data = list(mdata), 
		dexp = dexp,
		runconfig = NULL,
		columns = colnames(mdata)[-1],
		facetcolumns = NULL,
		ggplotaddon = list(ggplot2::theme(legend.position="bottom", legend.box='vertical'),
				ggplot2::scale_colour_discrete(guide=ggplot2::guide_legend(title.position="top", ncol=3),
