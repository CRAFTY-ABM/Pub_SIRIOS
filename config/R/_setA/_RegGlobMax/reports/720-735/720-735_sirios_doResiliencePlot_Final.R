...
#				- Select metrics in set
#				- Adjust setSigns accordingly
##########################################################################################
...
# considered metrics:
set <- c("UnderSupply_Total", "RegUnderSupply_Cereal", "ConsProp_NC", "NumActions")
...
fx["TriggerThreshold", "effects"] <- fx["TriggerThreshold", "effects"] * 0.75
fx["TriggerThreshold1", "effects"] <- fx["TriggerThreshold1", "effects"] * 0.75
fx["TriggerThreshold2", "effects"] <- fx["TriggerThreshold2", "effects"] * 0.75
fx["TriggerThreshold3", "effects"] <- fx["TriggerThreshold3", "effects"] * 0.75

##weights <- weights[c(23,24),]
# Weight effects with varying metric weights:
...
g <- ggplot2::ggplot(data, ggplot2::aes_string(set[3], set[4])) +
		...
		ggplot2::scale_x_continuous(metriclabels[set[3]], sec.axis = ggplot2::dup_axis(name = metriclabels[set[1]], breaks=NULL)) +
		ggplot2::scale_y_continuous(metriclabels[set[4]], sec.axis = ggplot2::dup_axis(name = metriclabels[set[2]], breaks=NULL))
