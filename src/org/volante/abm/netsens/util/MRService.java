 ...
 * MORe
 * 
 * Service class for R calculations - schedules at initialisation stopping R.Engine at the end of simulation - loads
 * igraph library - instantiates REngine
 * 
...
public class MRService implements RMainLoopCallbacks{
	 ...
	 * Constructor
	 */
	private MRService(final Schedule schedule) {
		// Stop R engine:

		schedule.register(new PostTickAction() {
	 ...
	 */
	static public MRService getInstance(Schedule schedule) {
		if (instance == null) {
			instance = new MRService(schedule);
		}
		return instance;
	}
	 ...
	 */
	public static Rengine getRengine(Schedule schedule) {
		Rengine re = Rengine.getMainEngine();
		if (re == null) {
			logger.debug("REngine-Version: " + Rengine.getVersion());
			re = new Rengine(R_ARGS, false, getInstance(schedule));
